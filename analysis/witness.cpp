...
// ---------------------------------------------------------------------------
// How much does PRUNING buy? A solver does not need the whole class -- it needs
// a WITNESS: the strategy DAG proving one root's value.
//   root WIN  : at our nodes pick ONE winning child; at theirs take ALL replies.
//   root DRAW : at our nodes pick ONE non-losing child; at theirs take ALL.
// The witness size against the class size is the pruning factor.
// ---------------------------------------------------------------------------
static size_t witnessSize(const Class& C, size_t root, uint8_t rootval){
    // Whose node it is is decided by side-to-move parity against the root,
    // NOT by the value: a DRAW node can belong to either player. At OUR nodes
    // one move suffices; at THEIRS every reply must be covered.
    Pos R; if(!decodeIdx(C, root, R)) return 0;
    uint8_t rootWtm = R.wtm;
    set<size_t> seen; vector<size_t> stack{root}; seen.insert(root);
    while(!stack.empty()){
        size_t id = stack.back(); stack.pop_back();
        Pos P; if(!decodeIdx(C, id, P)) continue;
        bool ours = (P.wtm == rootWtm);
        uint8_t v = C.val[id];
        if(ours){
            ...
            for(const Move& m : legalMoves(P)){
                ...
                int cap = P.b[m.to];
                if(cap && isWhite(cap)) continue;        // leaves the class -> leaf
                size_t qid = encodeIdx(C, Q);
                if(qid==SIZE_MAX) continue;
                uint8_t cv = C.val[qid];
                if(v==V_WIN  && cv==V_LOSS){ chosen=qid; break; }
                if(v==V_DRAW && cv==V_DRAW){ chosen=qid; break; }
            }
            if(chosen!=SIZE_MAX && !seen.count(chosen)){ seen.insert(chosen); stack.push_back(chosen); }
        } else {
            for(const Move& m : legalMoves(P)){
                Pos Q; legalAfter(P,m,Q);
                int cap = P.b[m.to];
                if(cap && isWhite(cap)) continue;
                size_t qid = encodeIdx(C, Q);
                if(qid==SIZE_MAX) continue;
                if(!seen.count(qid)){ seen.insert(qid); stack.push_back(qid); }
            }
        }
    }
    return seen.size();
...
int main(int argc, char** argv){
    ...
    if(argc>1 && string(argv[1])=="retro"){
        ...
        for(auto wp : chain){
            ...
            fflush(stdout);
            {   // Witness = the strategy DAG proving ONE root's value.
                // Sample densely; report the distribution, and flag witnesses
                // that escape the class immediately (their drawing move is a
                // capture into an already-solved lower class, so size 1 is an
                // artefact of the class boundary, not real pruning).
                vector<size_t> wsz, dsz; size_t esc=0;
                for(size_t id=0; id<C.size; id+=101){
                    uint8_t v=C.val[id];
                    if(v!=V_WIN && v!=V_DRAW) continue;
                    size_t z=witnessSize(C,id,v);
                    if(z<=1){ esc++; continue; }
                    if(v==V_WIN) wsz.push_back(z); else dsz.push_back(z);
                    if(wsz.size()+dsz.size()>3000) break;
                }
                auto rep=[&](const char* tag, vector<size_t>& v){
                    if(v.empty()){ printf("   %s witness: none sampled\n", tag); return; }
                    sort(v.begin(), v.end());
                    double mean=0; for(size_t x:v) mean+=x; mean/=v.size();
                    printf("   %s witness n=%4zu  median %7zu  mean %9.0f  max %8zu  = %.4f%% of class\n",
                           tag, v.size(), v[v.size()/2], mean, v.back(), 100.0*v.back()/(double)legal);
                };
                rep("WIN ", wsz); rep("DRAW", dsz);
                if(esc) printf("   (%zu sampled roots escape the class in one move - boundary artefact)\n", esc);
                fflush(stdout);
            }
