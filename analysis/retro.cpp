    ...
    if(argc>1 && string(argv[1])=="retro"){
        ...
        for(auto wp : chain){
                ...
                for(size_t id=0; id<C.size; id++){
                    ...
                    for(const Move& m : legalMoves(P)){
                        ...
                        if(cap && isWhite(cap)){
                            ...
                            if(it == done.end()){ printf("MISSING SUBCLASS\n"); return 3; }
                    ...
                    if(anyLoss){ C.val[id]=V_WIN; changed=true; }
                    else if(allWin){ C.val[id]=V_LOSS; changed=true; }
                }
                if(!changed) break;
            }
            size_t w=0,l=0,d=0;
            ...
            printf("class %-8s positions %9zu win %9zu loss %9zu draw %9zu "
                   "slots %10zu iters %2d time %7.2fs  %8.3f us/pos\n",
                   cname(wp).c_str(), legal, w, l, d, C.size, iters, secs,
                   legal? secs*1e6/(double)legal : 0.0);
            fflush(stdout);
