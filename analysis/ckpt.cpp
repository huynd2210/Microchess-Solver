...
// ===================================================================
// Disk-backed, checkpointable value store.
//
// WHY THIS IS SAFE, and why checkpointing needs almost no machinery:
// the retrograde update is MONOTONE -- a slot goes UNKNOWN -> WIN/LOSS
// and never changes again. So
//   * a half-flushed page loses updates but never corrupts one: every
//     byte is either its old or its new value, and both are legal;
//   * re-running any sweep from a partially converged array reaches the
//     same fixed point;
//   * therefore a crash needs no rollback. Resume, keep sweeping.
// The ONLY thing that must be recorded is whether pass 1 (legality and
// terminals) finished, because re-running pass 1 would reset solved
// slots back to UNKNOWN -- correct, but it would throw away the work.
// ===================================================================
struct Mapped {
    HANDLE hf = INVALID_HANDLE_VALUE, hm = nullptr;
    uint8_t* p = nullptr; uint64_t bytes = 0; string path;
    bool open(const string& fn, uint64_t sz, bool& existed){
        path = fn; bytes = sz;
        hf = CreateFileA(fn.c_str(), GENERIC_READ|GENERIC_WRITE, 0, nullptr,
                         OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
        if(hf==INVALID_HANDLE_VALUE) return false;
        existed = (GetLastError()==ERROR_ALREADY_EXISTS);
        DWORD t; DeviceIoControl(hf, FSCTL_SET_SPARSE, nullptr,0,nullptr,0,&t,nullptr);
        LARGE_INTEGER cur; GetFileSizeEx(hf, &cur);
        if((uint64_t)cur.QuadPart < sz){
            LARGE_INTEGER li; li.QuadPart=(LONGLONG)sz;
            SetFilePointerEx(hf, li, nullptr, FILE_BEGIN); SetEndOfFile(hf);
            existed = false;
        }
        hm = CreateFileMappingA(hf, nullptr, PAGE_READWRITE,
                                (DWORD)(sz>>32), (DWORD)(sz & 0xFFFFFFFF), nullptr);
        if(!hm) return false;
        p = (uint8_t*)MapViewOfFile(hm, FILE_MAP_ALL_ACCESS, 0,0,(SIZE_T)sz);
        return p != nullptr;
    }
    void flush(){ if(p){ FlushViewOfFile(p, 0); FlushFileBuffers(hf); } }
    void close(){ if(p) UnmapViewOfFile(p); if(hm) CloseHandle(hm);
                  if(hf!=INVALID_HANDLE_VALUE) CloseHandle(hf);
                  p=nullptr; hm=nullptr; hf=INVALID_HANDLE_VALUE; }
};

static string STORE_DIR;
static bool USE_STORE = false;

// progress file: one "done <class>" line per finished class, plus at most
// one "pass1 <class>" line. Written temp-then-rename so it is never torn.
static set<string> DONE_CLASSES;
static set<string> PASS1_DONE;
static void progressLoad(){
    FILE* f = fopen((STORE_DIR+"/progress.txt").c_str(), "r");
    if(!f) return;
    char kind[32], name[64];
    while(fscanf(f, "%31s %63s", kind, name)==2){
        if(!strcmp(kind,"done")) DONE_CLASSES.insert(name);
        else if(!strcmp(kind,"pass1")) PASS1_DONE.insert(name);
    }
    fclose(f);
}
static void progressSave(){
    string tmp = STORE_DIR+"/progress.tmp", fin = STORE_DIR+"/progress.txt";
    FILE* f = fopen(tmp.c_str(), "w");
    if(!f) return;
    for(const string& c : PASS1_DONE) fprintf(f, "pass1 %s\n", c.c_str());
    for(const string& c : DONE_CLASSES) fprintf(f, "done %s\n", c.c_str());
    fflush(f); fclose(f);
    MoveFileExA(tmp.c_str(), fin.c_str(), MOVEFILE_REPLACE_EXISTING|MOVEFILE_WRITE_THROUGH);
}
// partial results, appended as each class finishes: readable mid-run
static void resultAppend(const string& line){
    FILE* f = fopen((STORE_DIR+"/results.tsv").c_str(), "a");
    if(f){ fprintf(f, "%s\n", line.c_str()); fclose(f); }
}
...
static inline uint8_t vload(const uint8_t* v, size_t i){
    return __atomic_load_n(&v[i], __ATOMIC_RELAXED);
}
static inline void vstore(uint8_t* v, size_t i, uint8_t x){
...
struct Class {
    vector<int> wp;
    vector<uint8_t> mem;        // backing when not disk-backed
    Mapped mp;                  // backing when disk-backed
    uint8_t* val = nullptr;     // points into whichever is in use
    int k;
    size_t size = 0;
};
...
int main(int argc, char** argv){
    ...
    if(argc>1 && string(argv[1])=="retro"){
        ...
        for(int i=1;i<argc;i++) if(string(argv[i])=="--store" && i+1<argc){
            STORE_DIR = argv[i+1]; USE_STORE = true;
            CreateDirectoryA(STORE_DIR.c_str(), nullptr);
            progressLoad();
            printf("store %s : %zu classes already complete\n",
                   STORE_DIR.c_str(), DONE_CLASSES.size());
        }
        ...
        for(auto wp : chain){
            ...
            string cn = cname(wp);
            if(DONE_CLASSES.count(cn)){
                bool ex=false;
                if(!C.mp.open(STORE_DIR+"/"+cn+".val", C.size, ex)){ printf("cannot reopen %s\n", cn.c_str()); return 4; }
                C.val = C.mp.p;
                {
                    size_t w=0,l=0,d=0; u64 chk=1469598103934665603ULL;
                    for(size_t id=0; id<C.size; id++){
                        uint8_t v=C.val[id];
                        if(v==V_WIN) w++; else if(v==V_LOSS) l++; else if(v==V_DRAW) d++;
                        chk ^= v; chk *= 1099511628211ULL;
                    }
                    printf("class %-8s RESUMED win %9zu loss %9zu draw %9zu chk %016llx\n",
                           cn.c_str(), w, l, d, (unsigned long long)chk);
                }
                fflush(stdout);
                vector<int> k2 = wp; sort(k2.begin(), k2.end());
                done[k2] = std::move(C);
                continue;
            }
            bool existed = false;
            if(USE_STORE){
                if(!C.mp.open(STORE_DIR+"/"+cn+".val", C.size, existed)){ printf("cannot open store for %s\n", cn.c_str()); return 4; }
                C.val = C.mp.p;
                if(!existed) memset(C.val, V_ILLEGAL, C.size);
            } else {
                C.mem.assign(C.size, V_ILLEGAL);
                C.val = C.mem.data();
            }
            clock_t t0 = clock();
            ...
            bool skipPass1 = USE_STORE && existed && PASS1_DONE.count(cn);
            if(skipPass1){
                // pass 1 is deterministic; re-running it would reset solved
                // slots to UNKNOWN. Recount legal slots without touching values.
                size_t lc=0;
                for(size_t id=0; id<C.size; id++) if(C.val[id]!=V_ILLEGAL) lc++;
                legal_a = lc;
                printf("class %-8s resuming: pass 1 already done, %zu legal slots\n", cn.c_str(), lc);
                fflush(stdout);
            } else {
                run_parallel(NTHREADS, C.size, pass1);
                if(USE_STORE){ C.mp.flush(); PASS1_DONE.insert(cn); progressSave(); }
            }
            ...
            if(USE_STORE){
                C.mp.flush();
                DONE_CLASSES.insert(cn); PASS1_DONE.erase(cn); progressSave();
                char buf[256];
                snprintf(buf,sizeof buf,"%s	%zu	%zu	%zu	%zu	%d	%.2f",
                         cn.c_str(), legal, w, l, d, iters, wallsecs);
                resultAppend(buf);
            }
